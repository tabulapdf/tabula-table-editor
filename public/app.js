...
var TableView = Backbone.View.extend({
  ...
  _createSelectTool: function() {
    ...
    var rTmp = null;
    // extends the current selection from firstSelected to targetRectangle
    ...
    // draws the current selection, defined as the bounds of this.selectedRectangles
    var drawSelection = _.bind(function() {
      if (this.currentSelection !== null) this.currentSelection.remove();

      var selection = this.selectedRectangles.bounds,
          shrunkBounds = this.selectedRectangles.bounds.expand(this.selectedRectangles.bounds.width * -0.1,
                                                               this.selectedRectangles.bounds.height * -0.1);

      // this, my fellow programmer, is probably the worst code ever written.
      // Since I couldn't solve a geometric problem (the representation
      // makes it difficult), I Montecarlo-ed the shit out of it.
      // I will probably don't understand this a week from now.
      selection = _.reduce(_.range(75),
                           function(sel, i) {
                             return sel.unite(this._findDefinedRectangle(
                               new this.paperScope.Point((Math.random() * (selection.right - selection.left) + selection.left),
                                                         (Math.random() * (selection.bottom - selection.top) + selection.top))
                             ));
                           },
                           selection,
                           this);

      this.currentSelection = _.extend(new this.paperScope.Path.Rectangle(selection),
                                       this.currentSelectionStyle);
